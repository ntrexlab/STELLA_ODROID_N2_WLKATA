...
void pose_write_callback(const geometry_msgs::Pose msg)
{
	std::string Gcode = "";
	char poseX[10];
	char poseY[10];
	...
	sprintf(poseX, "%.2f", msg.position.x);
	sprintf(poseY, "%.2f", msg.position.y);
	sprintf(poseZ, "%.2f", msg.position.z);
	...
	if(HomingComplete == true && CanMove == true)
	{
		...
		while (ros::ok())
		{
			...
			MoveComplete = (result.data.npos == result.data.find("ok"))?false:true;
			if(MoveComplete == true)break;
		}
	}
...
void gripper_write_callback(const  std_msgs::Bool msg)
{
	std::string Gcode = "";

	if(msg.data==true) Gcode = (std::string)"M3S40" + "\r\n";
	else Gcode = (std::string)"M3S60" + "\r\n";
...
void grab_write_callback(const std_msgs::Bool msg)
{
	...
	{
		Gcode = (std::string)"M21 G90 G01 X0.00 Y45.70 Z18.40 A0.00 B-64.10 C0.00 F2000.00" + "\r\n";
		_serial.write(Gcode.c_str());
		while (ros::ok())
                {
                        Gcode = (std::string)"?" + "\r\n";
...
{
	ros::init(argc, argv, "mirobot_write_node");
	ros::NodeHandle nh;

	ros::Subscriber sub_pose = nh.subscribe("/pose", 1, pose_write_callback);
